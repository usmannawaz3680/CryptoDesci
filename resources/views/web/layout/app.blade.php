...
<body class="bg-black dark" @hasSection('sidebar')
        ...
        <nav>
            <div class="w-full flex flex-wrap items-center justify-between md:justify-start md:gap-8 p-4">
                ...
                <div class="hidden w-full justify-between items-center md:flex md:w-auto" id="navbar-dropdown">
                    <ul
                        class="flex flex-col font-normal p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:space-x-5 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-crypto-accent">
                        <li>
                            <a href="#"
                        ...
                        <li>
                            <button id="dropdownNavbarLink" data-dropdown-toggle="dropdownNavbar"
                                data-dropdown-trigger='hover' data-dropdown-delay=50
                            ...
                            <div id="dropdownNavbar"
                                class="z-50 hidden font-normal bg-white divide-y divide-gray-100 rounded-sm shadow-sm max-w-xs dark:bg-neutral-800 dark:divide-gray-600 p-4">
                                <div class="text-sm font-medium space-y-3 px-2">
                                    <a href="{{ route('web.tradingbots') }}" class="hover:text-crypto-primary group">
                                        ...
                                        <span
                                            class="text-gray-500 text-xs duration-75  group-hover:text-crypto-primary">Trade
                                            smarter with our various automated strategies - easy, fast and
                                            reliable</span>
                                    </a>
                                    <a href="{{ route('web.copytrading') }}" class="hover:text-crypto-primary group">
                                        ...
                                        <span
                                            class="text-gray-500 text-xs duration-75  group-hover:text-crypto-primary">Follow
                                            the most popular traders</span>
                                    </a>
                                </div>
                            </div>
                        </li>
                        ...
                        <li>
                            <a href="{{ url('/') }}"
                                class="block py-2 px-3 text-crypto-accent dark:text-white rounded-sm md:bg-transparent md:p-0 hover:bg-gray-700 md:hover:bg-transparent md:border-0 md:hover:text-crypto-primary dark:text-purple-400 md:dark:hover:text-crypto-primary dark:hover:text-white md:dark:bg-transparent">Donate</a>
                        </li>
                    </ul>
                </div>
                <!-- Icon Group -->
                <div class="hidden xl:flex items-center space-x-3 ml-auto">
                    <div>
                        <form action="{{ url('/') }}" method="get">
                            <input type="search" value="{{ request()->input('search', '') }}" name="search"
                                id="search"
                                class="flex-1 w-full text-sm bg-crypto-accent/70 border text-crypto-accent border-gray-700 rounded-md ps-3 pe-5 py-2 text-white placeholder-gray-400 focus:outline-none focus:border-crypto-primary focus:ring-1 focus:ring-crypto-primary">
                        </form>
                    </div>
                    <label for="search" class="cursor-pointer"><i
                            class="fas fa-search text-white hover:text-crypto-primary text-lg"></i></label>
                    @auth('web')
                        ...
                        <a href="{{ route('deposit') }}"
                            class="bg-crypto-primary text-black font-semibold px-3 py-1 rounded hover:bg-crypto-primary/80 transition">Deposit</a>
                        <div class="relative group">
                            <button class="flex items-center focus:outline-none">
                                <i class="fas fa-user text-white hover:text-crypto-primary text-lg"></i>
                                <svg class="w-3 h-3 ml-1 text-white group-hover:text-crypto-primary" fill="none"
                                    stroke="currentColor" stroke-width="2" viewBox="0 0 20 20">
                                    <path d="M6 8l4 4 4-4" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
                            </button>
                            <div
                                class="absolute -right-10 w-44 bg-white rounded-md shadow-lg z-20 hidden group-hover:block dark:bg-neutral-800">
                                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200">
                                    ...
                                    <li><a href="/wallet"
                                            class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700">Wallet</a>
                                    </li>
                                    <li>
                                        <form method="POST" action="{{ route('logout') }}">
                                            @csrf
                                            <button type="submit"
                                                class="w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700">Logout</button>
                                        </form>
                                    </li>
                                </ul>
                            </div>
                        </div>
                        <a href="{{ route('user.assets') }}"><i
                                class="fas fa-wallet text-white hover:text-crypto-primary text-lg"></i></a>
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" class="relative focus:outline-none">
                                <i class="fas fa-bell text-white hover:text-crypto-primary text-lg"></i>
                                @if (auth()->user()->unreadNotifications->count() > 0)
                                    <span
                                        class="absolute top-0 right-0 h-2 w-2 bg-crypto-primary rounded-full animate-ping"></span>
                                @endif
                            </button>

                            <div x-show="open" @click.away="open = false" x-transition
                                class="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-800 text-black dark:text-white rounded-lg shadow-lg z-50">
                                            ...
                                            <a href="{{ route('user.assets') }}"
                                                ...
                                                @if (!empty($notification->data['admin_note']))
                                                    <div class="text-xs text-gray-500 mt-1 italic">
                                                        ...
                                                        {{ number_format($notification->data['amount'], 4) }} transfer has
                                                        been {{ $notification->data['status'] }}
                                                    </div>
                                                @endif
                    ...
                    @if (auth('web')->guest())
                        <a href="/login"
                            class="bg-crypto-primary text-black font-normal px-3 py-1 rounded hover:bg-crypto-primary/80 transition">Login</a>
                        <a href="/register"
                            class="bg-crypto-primary text-black font-normal px-3 py-1 rounded hover:bg-crypto-primary/80 transition">Signup</a>
                    @endif
    ...
    <div class="flex min-h-screen pt-16">
        ...
        <!-- Main Content Area -->
        <main
            class="flex-1 overflow-y-auto bg-crypto-accent custom-scrollbar transition-all duration-300 ease-in-out @hasSection('sidebar')
@else
@endif"
            @hasSection('sidebar')
    ...
    <footer class="bg-crypto-accent/90 text-white py-8 transition-all duration-300 ease-in-out" @hasSection('sidebar')
        ...
        <div class="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-5 gap-8">
            <!-- Company Info -->
            <div>
                <h3 class="text-lg font-semibold text-crypto-primary mb-4">{{ env('APP_NAME') }}</h3>
                <p class="text-gray-400 text-sm">Your trusted platform for automated crypto solutions, offering copy
                    trading, bots, earning opportunities, and secure wallet management.</p>
    ...
    if (show) {
        ...
        <div x-show="type === 'success'"
            class="flex items-center w-full max-w-xs p-4 mb-4 text-green-800 bg-green-100 rounded-lg shadow"
            role="alert">
        ...
        <div x-show="type === 'error'"
            class="flex items-center w-full max-w-xs p-4 mb-4 text-red-800 bg-red-100 rounded-lg shadow"
            role="alert">
            <svg class="w-5 h-5 me-2 text-red-500" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd"
                    d="M18 10A8 8 0 11 2 10a8 8 0 0116 0zM9 9V5a1 1 0 012 0v4a1 1 0 01-2 0zm0 4a1 1 0 102 0 1 1 0 00-2 0z"
                    clip-rule="evenodd" />
            </svg>
            <span class="text-sm font-medium" x-text="message"></span>
        </div>
        <div x-show="type === 'warning'"
            class="flex items-center w-full max-w-xs p-4 mb-4 text-yellow-800 bg-yellow-100 rounded-lg shadow"
            role="alert">
            <svg class="w-5 h-5 me-2 text-crypto-primary" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd"
                    d="M8.257 3.099c.366-.773 1.396-.773 1.762 0l7.071 14.918c.33.695-.161 1.483-.881 1.483H2.067c-.72 0-1.211-.788-.881-1.483L8.257 3.1zM11 14a1 1 0 11-2 0 1 1 0 012 0zm-1-2a1 1 0 01-1-1V9a1 1 0 012 0v2a1 1 0 01-1 1z"
                    clip-rule="evenodd" />
            </svg>
            <span class="text-sm font-medium" x-text="message"></span>
        </div>
        <div x-show="type === 'info'"
            class="flex items-center w-full max-w-xs p-4 mb-4 text-blue-800 bg-blue-100 rounded-lg shadow"
            role="alert">
            <svg class="w-5 h-5 me-2 text-blue-500" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd"
                    d="M18 10A8 8 0 11 2 10a8 8 0 0116 0zM11 10a1 1 0 10-2 0 1 1 0 002 0zm0 2a1 1 0 10-2 0v2a1 1 0 002 0v-2z"
                    clip-rule="evenodd" />
            </svg>
            <span class="text-sm font-medium" x-text="message"></span>
        </div>
